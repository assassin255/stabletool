#!/usr/bin/env bash
set -e
#V4 — 2 changes: (1) removed -fno-math-errno (2) minimal build targets
silent() {
"$@" > /dev/null 2>&1
}
...
# V4: Build only essential targets (skip tests & extra tools = 38% fewer targets)
ninja -j"$(nproc)" qemu-system-x86_64 qemu-img
# Manual install since we only built 2 targets
...
# Copy BIOS/firmware files needed at runtime
